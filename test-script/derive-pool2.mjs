...
// Try: pool, config, base
const [pool1] = PublicKey.findProgramAddressSync([
  Buffer.from('pool'),
  configAddr.toBuffer(),
  baseMint.toBuffer()
], programId);
console.log(`['pool', config, base]: ${pool1.toString()} ${pool1.toString() === 'GGZ9ThN1c29d1AKmbyUkksnxV8SNmzBK6YGsfgRmX2b5' ? '✅ MATCH!' : ''}`);

// Try: pool, base, config  
const [pool2] = PublicKey.findProgramAddressSync([
  Buffer.from('pool'),
  baseMint.toBuffer(),
  configAddr.toBuffer()
], programId);
console.log(`['pool', base, config]: ${pool2.toString()} ${pool2.toString() === 'GGZ9ThN1c29d1AKmbyUkksnxV8SNmzBK6YGsfgRmX2b5' ? '✅ MATCH!' : ''}`);
